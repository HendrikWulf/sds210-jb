...
Reproducibility means that another person, or you at a later time, can rerun the same analysis with the same code, data, and environment and recover the same results.
...
* loading and reprojecting vector data to a common CRS
* clipping, masking, and filtering rasters
...
* the notebook file exists on your laptop
* the Python code looks readable
* a final figure was exported and saved once
* the repository was made public on GitHub
...
Suppose you build a notebook that extracts elevation values from a 25m DEM at various climate station locations, then maps the result.
...
A study can be reproducible without being replicable (e.g., the code runs perfectly, but the cooling effect only applies uniquely to Zurich's specific climate).
...
* test how sensitive the results are to different parameter choices
...
One of the most fatal habits is performing important preprocessing steps manually, outside of the code. Opening a `.csv` in Excel to delete a few "bad" rows, or clipping a layer interactively in QGIS and overwriting the original input file, breaks reproducibility immediately. There is no record of what was changed, making it impossible for anyone else to reproduce your work from the true raw data.

### Hidden notebook state

A notebook may only work on your machine because you ran the cells in a specific, hidden order (e.g., defining a variable at the bottom, then scrolling up to use it at the top). When someone else opens your file and clicks "Run All", the notebook immediately crashes.
...
If you hardcode these "magic numbers" without an inline comment explaining *why* you chose them, the workflow may still run, but the scientific reasoning becomes impossible to interpret.
...
Each of these save points, or commits, requires a short message explaining what changed and why. By writing helpful commit messages (e.g., "Add NDVI calculation for Landsat 8 imagery"), you turn your Git history into a traceable, readable log of your scientific development.
...
* **Portability:** There are no hardcoded absolute file paths.
...
* **Documentation:** A clear `README.md` explains the project, and notebooks tell a coherent narrative.
...
* data access restrictions (e.g., proprietary municipal data)
* massive file sizes (e.g., terabytes of raw satellite imagery)
...
* [ ] Are there zero hardcoded absolute file paths?
...
* [ ] Are variables named clearly and consistently?
* [ ] Are complex custom functions properly documented with docstrings?
* [ ] Are spatial assumptions (CRS choices, buffer distances, thresholds) explained in inline comments or Markdown?
* [ ] Are figure titles and captions informative, including provenance notes?
...
* **It differs from replicability:** Reproduibility tests the integrity of the workflow (same data, same methods); replicability tests the validity of the science (new data, same methods).
* **Threats are ordinary:** Most reproducibility failures come from small, invisible habits (like manual data edits or absolute paths), not dramatic coding errors.
