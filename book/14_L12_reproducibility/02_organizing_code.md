...
As spatial analysis grows, structure matters as much as syntax. Well organized code is easier to understand, easier to debug, and much easier to rerun on another machine or with another dataset.
```

In the previous chapter, you focused on readable code at the scale of the line and the cell. This chapter moves one level up: **workflow structure**.
...
1. load multiple vector and raster files
2. project them into a common CRS
...
Long, disorganized notebooks encourage bad habits, such as relying on "hidden states" (variables stored in memory from a cell you ran, then deleted 20 minutes ago) and manual interventions.
...
Imagine you need to buffer three different GeoDataFrames (stations, rivers, and roads) to perform an intersection. Without a function, you might just copy and paste the same logic:
...
Did you spot the spatial bug? In the code above, we accidentally applied a metric buffer (e.g., 500m) *before* projecting the data into a metric coordinate system (EPSG:2056). To fix this error, you now have to rewrite the logic in three different places!
...
As established in the [project guidelines](https://hendrikwulf.github.io/sds210-jb/book/projects/repository/#id-2-recommended-repository-structure), a professional, easy-to-follow repository explicitly separates your data, notebooks, and scripts:
...
Now, inside your Jupyter Notebook, you can import your custom tool exactly like you import standard libraries. *(Note: Because your notebook is inside the `notebooks/` folder, you must temporarily append the parent directory to your system path so Python can "see" the `src/` folder).*
...
Modules become incredibly powerful when your analysis spans multiple notebooks. 
...
* **Markdown cells** should explain the *why*: the research question, the assumptions, and the interpretation of the results.
...
A strong spatial notebook emphasizes exploration, interpretation, and communication. It should make it incredibly easy for a reader to answer five questions:  
1. What data came in?  
2. What changed?  
3. What was analyzed?  
