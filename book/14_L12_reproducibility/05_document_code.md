...
A reproducible project explains itself clearly enough that someone else can follow the logic, understand the assumptions, and rerun the workflow without guesswork. Code dictates *what* happens, but documentation explains *why* it happens.
...
* Why was this specific CRS or buffer distance chosen?
...
* use Jupyter notebooks as narrative documents rather than raw code dumps
* document reusable spatial functions with docstrings
* write clear inline comments that explain the *why*, not the *what*
...
The `README.md` file is the front door to your project. It is the first thing anyone sees when they open your repository on GitHub, GitLab, or a shared folder. In a well-organized project, it answers the most important questions before the reader even opens a notebook.
...
* **What data are used?** (And crucially, where to get it if it is too large to share)
* **What are the main workflow steps?** (Which scripts to run, and in what order)
* **What outputs are produced?** (Maps, tables, interactive plots)
...
* specific study areas and Coordinate Reference Systems (CRS)
* source datasets from different providers (and data access instructions)
* intermediate spatial transformations
* multiple output types like GeoTIFFs, Shapefiles, or PNG maps
...
* the chosen working CRS (e.g., EPSG:2056)
* minimum area filters (e.g., excluding buildings smaller than 100 m²)
* temporal aggregation rules (e.g., defining "Summer" as June 1 to August 31)
...
When you combine type hints with a clear docstring, your functions become incredibly easy for collaborators to use safely.
...
If a collaborator imports your `filter_high_elevation` function in a different notebook, they can simply type `help(filter_high_elevation)` (or hover their mouse over the function name in their editor), and your docstring will pop up. They never have to guess what data types to provide, what column names are required, or what the function will return.
...
## 4. Captions, figure titles, & provenances
...
Now the caption adds both provenance (reprojection, extraction) and interpretation (clustering).
...
When plotting in a notebook, ensure your figures contain descriptive titles, axis labels with units (e.g., are coordinates in degrees or meters?), and a small text addition noting where the data originated.
...
That single paragraph makes the notebook way more approachable.
...
Improve the project's documentation at both the macro (project) and micro (code) levels.
...
3. Add **type hints** to the parameters and the return value of the `clean_and_mask` function.
4. Write a standard **docstring** explaining what the function does, its parameters, and its return value.
5. Replace the weak code comments (which only explain the *what*) with strong comments that explain the *why* (the spatial assumptions).
...
* **Narrate your notebooks:** Use Markdown cells to guide the reader through the analytical story, providing geographical context before code and scientific interpretation after results.
